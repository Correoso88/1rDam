...
/*Exercici: especificació de paquet

Escriu l’especificació per un paquet anomenat “orders_mgmt” que
contengui els següents elements públics:
• Una funció per obtenir l’estat (status) d’una comanda (ORDERS),
donat el seu order_id.
• Una funció per obtenir tota la informació dels productes d’una
comanda, donada l’ID (order_id) d’aquesta.
• Els tipus de dades requerits per emprar les dues funcions
anteriors.*/

CREATE OR REPLACE PACKAGE ORDERS_MGMT AS

TYPE T_COLL_EMP IS TABLE OF EMPLOYEES%ROWTYPE;

FUNCTION get_statuss(order_id NUMBER) RETURN VARCHAR2;
FUNCTION get_products_by_order(p_order_id NUMBER) RETRUN T_COLL_EMP;
END ORDERS_MGMT;
