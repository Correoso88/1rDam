...
/*Utilitza un cursor implícit FOR per passar per tots els empleats
que tenguin “Programmer” com a JOB_TITLE.
2. Per cada programador, mira a quin trimestre de l’any 2016 es va
contractar (1r: 01 Jan – 31 Mar, 2n: 01 Apr – 30 Jun) i mostra un
missatge amb el nom del treballador i el trimestre de
contractació.*/

DECLARE

BEGIN
    FOR EMPLOYEE IN (SELECT * FROM EMPLOYES WHERE JOB_TITLE ='Programer') LOOP
        DBMS_OUTPUT.PUT_LINE(EMPLOYEES.FRIST_NAME||' '||EMPLOYEES.LAST_NAME);
        CASE WHEN EXTRACT(MONTH FROM EMPLOYEES.FRIST_DATE)IN(1,2,3)
        THEN DBMS_OUTPUT.PUT_LINE('1,T');
        END CASE;
    END LOOP;
END;
