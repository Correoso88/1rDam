...
/*Crea una funció de cerca d’empleats que, donat un varchar2, miri si
el first_name i last_name d’algun empleat coincideixen i en retorni
l’ID. Utilitza LIKE. Si hi ha més d’un empleat coincident retorna el
primer per ordre alfabètic.

Si no es troba cap empleat, aixeca una excepció que hagis definit
anteriorment.

Després, crea un bloc anònim que cerqui un empleat no existent i
gestiona l’excepció, mostrant un missatge “Employee not found”.*/

DECLARE 

    EMPLOYEES_NOT_FOUND EXCEPTION;

    FUNCTION CERCAR_EMPLEATS(P_NAME VARCHAR2)RETURN NUMBER;
    IS
    V_ID_EMPLOYEES NUMBER;
    BEGIN
    SELECT ID INTO V_ID_EMPLOYEES FROM EMPLOYEES WHERE FIRST_NAME LIKE '%'||P_NAME||'%' ORDER BY FETCH FIRST;
    EXCEPTION
        WHEN NOT_DATA_FOUND THEN
            RAISE EMPLOYEES_NOT_FOUND;
    END;
BEGIN
    DBMS_OUTPUT_PUT_LINE('ID: '||GET_EMPLOYEES_ID);
    EXCEPTION
    WHEN EMPLOYEES_NOT_FOUND
END;
