...
DECLARE
    vid_llibre NUMBER := 3;
    VEXEMPLARS llibre.exemplars%TYPE;
    VDNI VARCHAR2(9) := '69241569E';
BEGIN
    SELECT EXEMPLARS INTO VEXEMPLARS FROM LLIBRE WHERE VID_LLIBRE = ID;
    WHILE I < VEXEMPLARS LOOP
    UPDATE LLIBRE SET exemplars = exemplars -1 WHERE VID_LLIBRE = ID;
    INSERT INTO VENDA(DNI_CLIENT, ID_LLIBRE, DATA_VENDA, PREU)VALUES(VDNI, vid_llibre, SYSTIMESTAMP, 20);
    END LOOP;
END;
