...
DECLARE
VEMPLOYEDID CONSTANT NUMBER := 44;
VJOB_TITLE EMPLOYEES.JOB_TITLE%TYPE;
VHIREDATE DATE;
BEGIN
    SELECT JOB_TITLE, HIRE_DATE INTO VJOB_TITLE, VHIREDATE FROM EMPLOYEES WHERE EMPLOYEES_ID = VEMPLOYEDID;
    IF VJOB_TITLE != 'Programmer' THEN
        DBMS_OUTPUT.PUT_LINE('El trabajador no es programador');
    ELSE
        CASE
            WHEN EXTRACT(MONTH FROM VHIREDATE) IN (1, 2, 3) THEN
                DBMS_OUTPUT.PUT_LINE('Prime trimestre');
            WHEN EXTRACT(MONTH FROM VHIREDATE) IN (4, 5, 6) THEN
                DBMS_OUTPUT.PUT_LINE('Segon trimestre');
            WHEN EXTRACT(MONTH FROM VHIREDATE) IN (7, 8, 9) THEN
                DBMS_OUTPUT.PUT_LINE('Tercer trimestre');
            ELSE
                DBMS_OUTPUT.PUT_LINE('Quarto trimestre');
        END CASE;
    END IF;
END;