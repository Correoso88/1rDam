...
DECLARE
TYPE T_VARR IS VARRAY(3) OF LLIBRE%ROWTYPE;
L_VARR T_VARR;
BEGIN
SELECT * BULK COLLECT INTO L_VARR FROM LLIBRE ORDER BY EXEMPLARS DESC FETCH FIRST 3 ROWS ONLY;
FOR i in l_varr.first..lvarr.last loop
   DBMS_OUTPUT.PUT_LINE(L_VARRi.frist_name || ' ' ||l_varr(i).last_name); 
   END LOOP;
END;