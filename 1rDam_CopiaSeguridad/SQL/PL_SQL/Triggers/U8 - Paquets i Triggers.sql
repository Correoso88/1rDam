...
CREATE PACKAGE LLIBRES_API AS 
    --TYPES
    TYPE LLIB_REC LLIBRE%ROWTYPE;

    --SUPROGRAMAS
    PROCEDURE alta_llibre(TITOL VARCHAR2, ANY NUMBER, EXEMPLARS NUMBER, ID_EDITORIAL NUMBER);

    FUNCTION get_llibre_by_id(V_NOM_GENERE NUMBER)RETURN LLIB_REC IS

    BEGIN
        SELECT * BULK COLLECT FROM LLIBRE_GENERE WHERE NOM_GENERE = V_NOM_GENERE ORDER BY TITOL;
    END;

    FUNCTION search_by_titol() RETURN;
