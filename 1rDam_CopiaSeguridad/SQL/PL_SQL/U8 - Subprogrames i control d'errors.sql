...
DECLARE
    TYPE T_VARR IS TABLE OF LLIBRE%ROWTYPE;
    L_VARR T_VARR;

    FUNCTION GET_LLIBRES_BY_AUTOR_ID(P_ID NUMBER)RETURN T_VARR IS
    L_RESULT T_VARR;
    BEGIN
    SELECT * BULK COLLECT INTO L_VARR FROM LLIBRE JOIN AUTOR_LLIBRE ON LLIBRE.ID = AUTOR_LLIBRE.ID_LLIBRE WHERE AUTOR_LLIBRE.ID_AUTOR = P_ID;
    RETURN L_RESULT;
    END;
BEGIN
    L_VARR := GET_LLIBRES_BY_AUTOR_ID(1);

    FOR i IN 1 .. L_VARR.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE(L_VARR(i).TITOL);
    END LOOP;
END;
