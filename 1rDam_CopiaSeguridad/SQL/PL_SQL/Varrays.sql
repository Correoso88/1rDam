...
--Sobre “HR.sql”
--1. Guarda els noms dels 3 empleats amb més comandes (order)
--shipped a un Varray ordenant-los de més a menys.
--2. Mostra el nom dels empleats (DBMS_OUTPUT).
--3. Elimina (TRIM) el 2n i 3r empleats amb més comandes shipped i
--comprova que el nombre final d’elements al Varray és 1. Mostra el
--nom de l’empleat que queda al Varray.

DECLARE
    TYPE T_VARR IS VARRAY(3) OF VARCHAR2(50);
    V_ARR T_VARR;
BEGIN
    SELECT EMPLOYEES.FIRST_NAME BULK COLLECT INTO V_ARR FROM EMPLOYEES JOIN ORDERS ON ORDERS.SALESMAN_ID = EMPLOYEES.EMPLOYEE_ID WHERE ORDERS.STATUS = 'Shipped' group by employees.frist_name ORDER BY COUNT(*) DESC FETCH FIRST 3 ROWS ONLY;
    FOR IDX IN 1..3 LOOP
    DBMS_OUTPUT.PUT_LINE(idx||': '||v_arr(idx));
    END LOOP;
    V_ARR.TRIM(2);
    DBMS_OUTPUT.PUT_LINE('Mida: ' ||v_arr.count);
    DBMS_OUTPUT.PUT_LINE(v_arr(v_arr.LAST));
END;